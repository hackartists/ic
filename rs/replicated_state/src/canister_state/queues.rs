...
pub struct CanisterQueues {
    ...
    /// The callback IDs of all responses enqueued in input queues, whether an
...
impl CanisterQueues {
    ...
    fn skip_canister_input(&mut self, input_queue_type: InputQueueType) {
        if let Some(sender) = self.input_schedule.peek(input_queue_type) {
            // FIXME Add a test for skipping a GC-ed queue.
            // If the input queue is non-empty, re-enqueue the sender at the back of the
            // input schedule queue.
            if self
                .canister_queues
    ...
    fn schedules_ok(
        ...
        self.input_schedule
            .test_invariants(&self.canister_queues, &input_queue_type_fn)
    }
    ...
    /// `debug_assert_eq!(Ok(()), self.input_queue_ok(...)`.
    ///
...
/// queue is either empty or starts with a non-stale reference, by writing
/// `debug_assert_eq!(Ok(()), output_queue_ok(...)`.
///
/// Time complexity: `O(log(n))`.
fn output_queue_ok(
    queue: &CanisterQueue,
...
fn callbacks_with_enqueued_response(
    ...
    if callbacks.len() != callbacks_vec.len() {
        return Err(format!(
            "CanisterQueues: Duplicate inbpund response callback(s): {:?}",
            callbacks_vec
        ));
    }
